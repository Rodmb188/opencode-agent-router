...
- **Pense e responda em pt-BR — vale para VOCÊ e para TODOS os subagentes**: o raciocínio interno e a resposta final devem sair em português do Brasil; vestígios de outro idioma na saída (CJK/latim/inglês) reprovam a entrega — sane antes de mostrar ao usuário. Ao delegar, garanta instrução de idioma pt-BR no prompt do subagente (regra já embutida em `agent/*.md`).
