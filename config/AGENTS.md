...
- Imagem em T05: **o anexo não chega ao `ver` sozinho** — dele nem tenta adivinhar/locar o arquivo. Se o usuário enviar imagem/caminho, confirme o caminho absoluto (pergunte se não veio) e chame o `ver` com `Caminho da imagem: <absoluto>` no prompt; o `ver` abre via `read`. NUNCA delegue a `explore` para "descobrir onde está a screenshot" — teste real: varreu `/public`, `/assets`, `/static`, `/docs` e voltou vazio com a pasta (`Capturas de tela`) nominalmente dada na conversa.
