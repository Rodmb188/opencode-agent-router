...
- **Idioma pt-BR com rigor máximo — vale para VOCÊ e para TODOS os subagentes**: toda saída (raciocínio e resposta) em português, só caracteres latinos. Vazamentos CJK são artefato de decodificação e reprovam a entrega — o smoke T16 r5 mostrou `颗`, `字词句修正如下`, `润色后`, `分步骤思考` e `丄` mesmo com as regras ativas. Dever do primário: **inspecionar a saída completa do subagente (início, meio e fim)** e sanitizar tudo antes de mostrar; nunca repassar resposta com caracteres estrangeiros. Ao delegar, inclua a instrução de idioma no prompt (já embutida em `agent/*.md`).
